      ...
      <div className="grid grid-cols-1 md:grid-cols-[1fr_auto_1fr] gap-8 items-start">
        ...
        <div>
          <div className="flex items-center gap-2 mb-3">
            <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-gray-800 shrink-0">
          ...
          <p className="text-gray-500 text-sm leading-relaxed">
            ...
            Memberdayakan Umat dan Menebarkan Maslahat Secara Berkelanjutan.
          </p>
        </div>

        {/* Divider */}
        <div className="hidden md:flex justify-center self-stretch">
          <div className="w-px bg-gray-400 h-full rounded-full" />
        </div>
            ...
            <div className="relative z-10 bg-[#213f9a] rounded-2xl px-8 py-10 flex items-center justify-around gap-4 mx-10">
              {tabs.map((label) => (
                <button
                  ...
                  className="flex flex-col items-center gap-2 text-white text-center min-w-0 group"
                >
                  <span className={`text-base font-semibold leading-tight transition-opacity duration-200 ${activeTab === label ? "opacity-100" : "opacity-60 group-hover:opacity-80"}`}>
                    {label}
                  </span>
                  <span className={`h-0.5 rounded-full bg-white transition-all duration-300 ${activeTab === label ? "w-full" : "w-0 group-hover:w-full"}`} />
                </button>
              ))}
            </div>
